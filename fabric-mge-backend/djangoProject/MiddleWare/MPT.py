...
from djangoProject.MiddleWare.Hash import keccak_hash
from djangoProject.MiddleWare.Nibble_path import NibblePath
from djangoProject.MiddleWare.Node import Node
...
class MerklePatriciaTrie:
    ...
    def update_mge(self, encoded_key, encoded_value):
        try:
            data = self.get(encoded_key)
            data_str = data.data.decode('utf-8')
            value = data_str + '/div/' + encoded_value
            self.update(encoded_key, value)
        except KeyError:
    ...
    def _update(self, node_ref, path, value):
        ...
        elif type(node) == Node.Extension:  # 扩展节点
            if path.starts_with(node.path):
        ...
        elif type(node) == Node.Branch:  # 分支节点
            if len(path) == 0:
