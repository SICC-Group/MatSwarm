...
func (t *EducationChaincode) Invoke(stub shim.ChaincodeStubInterface) peer.Response {
	...
	} else if fun == "findMgeData" {
		return t.findMgeData(stub, args)
	} else if fun == "addData" { // 添加mge数据
		return t.addData(stub, args)
	} else if fun == "addTemplate" { // 添加mge模板
		return t.addTemplate(stub, args)
	} else if fun == "QueryMgeData" { // query
		return t.QueryMgeData(stub, args)
	} else if fun == "FinishAddData" {
		return t.FinishAddData(stub, args)
	} else if fun == "LoginUserEvent" {
		return t.LoginUserEvent(stub, args)
	} else if fun == "FCtrain" {
	    return t.FCtrain(stub, args)
	}
	return shim.Error("指定的函数名称错误111")
	// return t.recordLoginEvent(stub, args)
}
